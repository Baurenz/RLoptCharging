...
for bus, comps in components.items():
    for comp_key, comp_value in comps.items():
        ...
        for day in range(days):
            ...
            if comp_key == "pv":
                day_profile = pv_profile(day_time_index, comp_value)
            elif comp_key == "load":
                if comp_value == "load1":
...
for bus, comps in components.items():
    ...
    for comp_key, comp_value in comps.items():
        if comp_key not in ["pv", "load"]:  # Only proceed if the component is 'pv' or 'gen'
            continue
        ...
        filtered_profile = profile[(profile.index >= pd.to_datetime(start_date)) & (
                    profile.index < pd.to_datetime(start_date) + pd.Timedelta(days=5))]
