...
def simulate_control(self, actions):
    ...
    leave = self.leave
    boc = self.soc_cs
    p_charging = np.zeros(self.n_cars_bus)

    for car in range(self.n_cars_bus):
        if actions[car] >= 0:
    ...
    if self.simnet_flag:
        net_result_dict = run_single_timestep_pf(self.net, time_step=self.timestep,
                                                 profile_data_dict=self.profile_data_dict,
    ...
    #
    if self.simnet_flag:
        cost_1 = grid_final * self.energy_price_glob[0, hour]
