...
selected_cars = [0, 1]
session_prefix = "11_bus_EV5_EP10_SS0"
session_prefix = "no_selling_Energy_4_bus_simple_noLoad_noV2G_EV0_EP10_SS30_"
session_id = 12
...
def get_available_episodes(session_prefix, session_id):
    episode_list = []
    base_path = f'rl_charging_station/files/Results/{session_prefix}{session_id}/'
...
def update_episode(episode):
    try:
        ...
        if episode in available_episodes:
            ...
            initial_values_json = load_json(f'rl_charging_station/files/Results/{session_prefix}{session_id}/Initial_Values_{episode}.json')
            results_json = load_json(f'rl_charging_station/files/Results/{session_prefix}{session_id}/results_{episode}.json')
...
available_episodes = get_available_episodes(session_prefix, session_id)
...
def plot_data(renewable_power, load_power, power_ess, power_cs, departure_t, arrival_t, day_ahead_price, day_ahead_date, soc_ess, soc_cs,
    ...
    line_cost_ext_grid = [ax2_cost_ext_grid.plot(x_96, cost_ext_grid, label='Cost Ext Grid', color='green', linestyle=':')[0]]
    ...
    line_power_ext_grid = [
        ax2_power_ext_grid.plot(x_96, power_ext_grid, label='Power Ext Grid', color='blue', linestyle='-.')[0]]  # as a list
    ...
    for i, car_idx in enumerate(selected_cars):
        ...
        minor_ticks = x_96  # Assuming 96 points for 24 hours
        ...
        lines_soc_cs.append(axs[i + 1].plot(x_97, soc_cs[car_idx], label=f'soc_cs Car {car_idx + 1}', **label_styles['soc_cs'])[0])
        if soc_ess != 0:
            lines_soc_ess.append(axs[i + 1].plot(x_97, soc_ess[car_idx], label=f'soc_ess Car {car_idx + 1}', **label_styles['soc_ess'])[0])
            lines_power_ess.append(ax2.plot(x_96, power_ess[car_idx], label=f'power_ess', **label_styles['power_ess'])[0])
        ...
        lines_power_cs.append(ax2.plot(x_96, power_cs[car_idx], label=f'power_cs', **label_styles['power_cs'])[0])
...
initial_values_json = load_json(f'rl_charging_station/files/Results/{session_prefix}{session_id}/Initial_Values_{initial_episode}.json')
results_json = load_json(f'rl_charging_station/files/Results/{session_prefix}{session_id}/results_{initial_episode}.json')
