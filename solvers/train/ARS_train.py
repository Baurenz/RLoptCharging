...
from sb3_contrib import ARS  # Import ARS from sb3_contrib
...
session_prefix = "ARS_weight_to_wastePV"
session_id = 2
...
base_models_dir = Path("../models/ARS")
base_logdir = Path("../results/logs/ARS")
...
if session_id is not None and (latest_model_path := get_latest_model(models_dir)):
    model = ARS.load(latest_model_path, env=env)
    print(f"Resuming training from: {latest_model_path}")
else:
    print("Starting new training session.")
    model = ARS("MlpPolicy", env, verbose=1, tensorboard_log=str(logdir))

# Training loop
TIMESTEPS = 20000
for i in range(1, 50):
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name="ARS")
    model.save(f"{models_dir}/ARS{TIMESTEPS * i}")
    if not debug_flag:
        create_profiles.create_new_profiles(network_config_path)
