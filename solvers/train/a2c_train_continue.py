...
session_prefix = "penalcs_4_bus_simple_noLoad_noV2G_EV0_EPb10_EPs2_SS30_"
session_id = 1
...
base_models_dir = Path("../models/A2C")  # Changed from PPO to A2C
base_logdir = Path("../results/logs/A2C")  # Changed from PPO to A2C
...
# Load the latest model if continuing training or initialize a new A2C model
if session_id is not None:
    latest_model_path = get_latest_model(models_dir)
    if latest_model_path:
        model = A2C.load(latest_model_path, env=env)  # Changed from PPO to A2C
        ...
        model = A2C("MlpPolicy", env, verbose=1, tensorboard_log=str(logdir))  # Changed from PPO to A2C
else:
    model = A2C("MlpPolicy", env, verbose=1, tensorboard_log=str(logdir))  # Changed from PPO to A2C
...
TIMESTEPS = 20000
for i in range(1, 50):
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name="A2C", callback=callback)  # Changed tb_log_name from PPO to A2C
    model.save(f"{models_dir}/A2C_{TIMESTEPS * i}")  # Changed filename from PPO to A2C
    create_profiles.create_new_profiles(network_config_path)
