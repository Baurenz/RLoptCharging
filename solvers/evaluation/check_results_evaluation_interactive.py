...
label_styles = {
    ...
    'remaining_pv': {'color': 'pink', 'linestyle': '-'},
...
def extract_data_from_json(initial_values_json, results_json):
    ...
    renewable_power = np.array([renewable_dict[str(i)] if str(i) in renewable_dict else [None] * 97 for i in range(10)])
    ...
    # Ensure present_cars matches the number of timesteps in power_cs by cutting the last value
    ...
    num_ess = soc_ess.shape[0]
    power_ess = np.array(results_json['power_ess']).reshape(-1, num_ess).T
...
def update_episode(episode):
    global available_episodes
    global selected_model
    global session_prefix
    global session_id

    print(f'"avalable episodes:{available_episodes}')

    try:
        episode = int(episode)  # Convert entered text to an integer
        if episode in available_episodes:
            # Load data for the new episode
            initial_values_json = load_json(
                f'solvers/evaluation/Results/{selected_model}_{session_prefix}{session_id}/Initial_Values_{episode}.json')
            results_json = load_json(f'solvers/evaluation/Results/{selected_model}_{session_prefix}{session_id}/results_{episode}.json')
...
def update_plot(label):
    ...
    graphs = {
        ...
        'load': lines_load,
        'remaining_pv': lines_remaining_pv,
...
def update_legend(label_styles):
    labels = set()
    all_lines = lines_soc_cs + lines_soc_ess + lines_renewable + lines_remaining_pv + lines_load + lines_power_ess + lines_power_cs
...
def plot_data(renewable_power, remaining_pv, load_power, power_ess, power_cs, departure_t, arrival_t, day_ahead_price, day_ahead_date,
    ...
    global lines_soc_cs, lines_soc_ess, lines_renewable, lines_remaining_pv, lines_load, lines_power_ess, lines_power_cs, line_power_ext_grid, line_cost_ext_grid, line_day_ahead_price
    global ax2_cost_ext_grid, ax2_power_ext_grid
    ...
    twin_axes = []
    axs[0].clear()
    line_day_ahead_price = [axs[0].plot(x_97, day_ahead_price, label='Day-Ahead Price', color='purple', drawstyle='steps-post')[0]]
    ...
    for i, car_idx in enumerate(selected_cars):
        ...
        if soc_ess.any() != 0:
            ...
            lines_power_ess.append(
                ax2.plot(x_97, power_ess[car_idx][:97], label=f'power_ess', **label_styles['power_ess'], drawstyle='steps-post')[0])

        lines_renewable.append(
            ax2.plot(x_97, renewable_power[car_idx], label=f'renewable', **label_styles['renewable'], drawstyle='steps-post')[0])

        lines_remaining_pv.append(
            ax2.plot(x_97, remaining_pv[car_idx], label=f'remaining_pv', **label_styles['remaining_pv'], drawstyle='steps-post')[0])

        if load_power.any():
            lines_load.append(ax2.plot(x_97, load_power[car_idx], label=f'load', **label_styles['load'], drawstyle='steps-post')[0])

        lines_soc_cs.append(axs[i + 1].plot(x_97, soc_cs[car_idx][:97], label=f'soc_cs Car {car_idx + 1}', **label_styles['soc_cs'])[0])
        lines_power_cs.append(ax2.plot(x_97, power_cs[car_idx], label=f'power_cs', **label_styles['power_cs'], drawstyle='steps-post')[0])

        for line_list, state in zip(
                [lines_soc_cs, lines_soc_ess, lines_renewable, lines_remaining_pv, lines_load, lines_power_ess, lines_power_cs],
...
checkbox_labels = ['day_ahead_price', 'soc_cs', 'soc_ess', 'renewable', 'remaining_pv', 'load', 'power_ess', 'power_cs', 'power_ext_grid',
                   'cost_ext_grid']
...
def run_interactive_analysis(selected_model_param, session_prefix_param, session_id_param):
    global available_episodes, selected_model, session_prefix, session_id

    selected_model = selected_model_param
    session_prefix = session_prefix_param
    ...
    initial_values_json = load_json(
        f'solvers/evaluation/Results/{selected_model}_{session_prefix}{session_id}/Initial_Values_{initial_episode}.json')
    results_json = load_json(f'solvers/evaluation/Results/{selected_model}_{session_prefix}{session_id}/results_{initial_episode}.json')
...
if __name__ == "__main__":
    ...
    # selected_model = 'DDPG'
    # session_prefix = "jax_6bus_epb4_ss600"
    # session_id = 1
    #
    # available_episodes = get_available_episodes(selected_model, session_prefix, session_id)
    # # Plot the initial data
